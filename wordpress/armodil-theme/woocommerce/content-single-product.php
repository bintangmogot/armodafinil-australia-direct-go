...
function armo_custom_reviews_anchor_button()
{
    ...
    $average_rating = $valid_ratings > 0 ? round($total_rating / $valid_ratings, 2) : 5.00;
    $r = intval(round($average_rating));

    echo '<div class="my-2 lg:mt-3 lg:mb-4">';
    echo '<a href="#product-reviews-section" class="inline-flex flex-col gap-1 lg:gap-1.5 bg-gradient-review hover:bg-secondary-dark text-white px-4 py-3 rounded-lg transition-colors w-full md:w-auto md:max-w-[280px]">';
    ...
    echo '</div>';
    echo '<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 lg:h-5 lg:w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z" clip-rule="evenodd" /></svg>';
    echo '</div>';
    
    // Bottom row: Text
    $rating_display = fmod($average_rating, 1) !== 0.00 ? number_format($average_rating, 2) : number_format($average_rating, 0);
    echo '<div class="text-xs lg:text-sm leading-snug text-left">';
    echo '<strong>Rated ' . esc_html($rating_display) . '</strong> out of <strong>5</strong> based on verified customer reviews';
    echo '</div>';
...
function armo_custom_total_price_box()
{
    ...
    if ($product->is_type('variable')) {
        ...
        echo '<div class="text-[11px] md:text-xs text-teal-700 font-semibold mb-0.5">Total Price</div>';
...
<div id="product-<?php the_ID(); ?>" <?php wc_product_class('custom-product-layout flex flex-col max-w-7xl mx-auto w-full', $product); ?>>
        ...
        <div class="armo-product-tabs mt-0 lg:mt-8 mb-16">
            ...
            <div class="armo-tabs-nav-wrapper">
                <nav class="armo-tabs-nav" aria-label="Product Tabs" id="product-tabs-nav">
                    <?php if ($description): ?>
                        <button class="armo-tab-btn" data-target="tab-description" type="button">
                            Description
                        </button>
                    <?php endif; ?>
                    <?php if ($extra_tabs): ?>
                        <?php foreach ($extra_tabs as $index => $tab): ?>
                            <button class="armo-tab-btn" data-target="tab-extra-<?php echo $index; ?>" type="button">
            ...
            <div class="armo-tabs-content" id="product-tabs-content">
                <?php if ($description): ?>
                    <div class="armo-tab-pane prose max-w-none text-teal-700-dark" id="tab-description">
                        <?php echo apply_filters('the_content', $description); ?>
                    </div>
                <?php endif; ?>
                <?php if ($extra_tabs): ?>
                    <?php foreach ($extra_tabs as $index => $tab): ?>
                        <div class="armo-tab-pane prose max-w-none text-teal-700-dark" id="tab-extra-<?php echo $index; ?>">
    ...
    <div class="armo-product-reviews mt-8 lg:mt-12 mb-8" id="product-reviews-section"
        data-product-id="<?php echo esc_attr($product_id); ?>">
        <h2 class="text-2xl lg:text-3xl font-bold text-teal-700 mb-8 pb-4 border-b border-primary/10">
            Customer Reviews
        </h2>
            ...
            <div class="flex flex-col lg:flex-row gap-8 lg:gap-10">
                ...
                <div class="lg:w-1/4 flex flex-col">
                    <div class="text-xs text-gray-500 mb-6 uppercase tracking-wider font-semibold"
                        id="product-reviews-count-display">
                        Showing <?php echo esc_html($shown_count); ?> of <?php echo esc_html($total_product_reviews); ?>
                        reviews
                    </div>
                ...
                <div class="lg:w-3/4 flex flex-col gap-6" id="product-reviews-wrapper">
                    ...
                    <div class="relative w-full">
                        <!-- Carousel Buttons -->
                        <button type="button" id="review-prev" class="absolute top-1/2 -translate-y-1/2 bg-white rounded-full p-2 shadow-xl z-10 text-teal-700 hover:text-accent focus:outline-none hidden md:flex items-center justify-center" style="left: -3rem;">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                        </button>
                        <button type="button" id="review-next" class="absolute top-1/2 -translate-y-1/2 bg-white rounded-full p-2 shadow-xl z-10 text-teal-700 hover:text-accent focus:outline-none hidden md:flex items-center justify-center" style="right: -3rem;">
                    ...
                    <?php if ($review_query->max_num_pages > 1): ?>
                        <div id="product-reviews-pagination" class="mt-8 flex flex-col items-center">
                            ...
                            <button type="button" id="product-reviews-load-more"
                                ...
                                data-page="1" data-product-id="<?php echo esc_attr($product_id); ?>">
                                Load More
        ...
        <div class="mt-8 relative py-8 px-2 md:px-4 lg:px-6">
            ...
            <div class="absolute inset-0 bg-teal-600" style="left: 50%; transform: translateX(-50%); width: 100vw;">
            </div>
            ...
            <div class="max-w-xl mx-auto text-center relative z-10">
                ...
                <div class="bg-surface rounded-md md:rounded-xl p-4 lg:p-6 text-left shadow-xl">
                    <form id="armo-product-review-form" method="POST" class="flex flex-col gap-3">
                        ...
                        <!-- Rating -->
                        <div>
                            <label class="block text-sm font-bold text-teal-700 mb-1">Your overall rating *</label>
                        ...
                        <!-- Title -->
                        <div>
                            <label class="block text-sm font-bold text-teal-700 mb-1">Title of your review *</label>
                            <input type="text" name="review_title" placeholder="Enter your title"
                                class="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:border-primary transition-colors"
                                required>
                        </div>

                        <!-- Review Content -->
                        <div>
                            <label class="block text-sm font-bold text-teal-700 mb-1">Your review *</label>
                            <textarea name="review_content" placeholder="Write your review" rows="3"
                                class="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:border-primary transition-colors resize-none"
                                required></textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <!-- Name -->
                            <div>
                                <label class="block text-sm font-bold text-teal-700 mb-1">Your name *</label>
                            ...
                            <!-- Email -->
                            <div>
                                <label class="block text-sm font-bold text-teal-700 mb-1">Your email address *</label>
                                <input type="email" name="review_email" placeholder="Enter your email"
                                    class="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:border-primary transition-colors"
                                    required>
                            </div>
                        </div>
                        ...
                        <div id="product-review-form-message"
                            class="hidden rounded-lg p-3 text-sm font-medium text-center"></div>

                        <!-- Submit Button -->
                        <div class="mt-1 text-center">
                            <button type="submit" id="product-review-submit-btn"
                                class="bg-[#FF0000] text-white font-bold py-2 px-6 rounded-full hover:bg-red-600 transition-colors w-full md:w-auto text-sm">
                                Submit your review
                            </button>
                        </div>
                    </form>
