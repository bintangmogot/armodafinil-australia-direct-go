...
<section class="py-12 lg:py-16 px-6 lg:px-12 <?php echo $background_color ? '' : 'bg-surface'; ?> overflow-hidden" <?php if($background_color) echo 'style="background-color: ' . esc_attr($background_color) . ';"'; ?>>
    <div class="max-w-4xl mx-auto text-center">
        ...
        <?php if ($subheading) : ?>
            <h2 class="text-xl md:text-2xl font-bold text-teal-700 mb-6"><?php echo esc_html($subheading); ?></h2>
        <?php endif; ?>

        <?php if ($content) : ?>
            <div class="module-city-header-content text-teal-700 text-base md:text-lg leading-relaxed space-y-4 mb-8">
                <?php echo armo_content($content); ?>
            </div>
        <?php endif; ?>

        <?php if ($delivery_text) : ?>
            <div class="flex items-center justify-center gap-2 font-bold text-teal-700 mb-4">
        ...
        <?php if ($button_text && $button_link) : ?>
            <a href="<?php echo esc_url($button_link); ?>" class="group inline-flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-700 text-white font-bold text-base md:text-lg px-6 md:px-8 py-3 rounded-md transition-colors shadow-md no-underline">
